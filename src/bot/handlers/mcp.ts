/**
 * /mcp — inspect and control the OpenCode agent's MCP servers from Telegram.
 *
...
export function registerMcp(bot: Bot, deps: BotDeps): void {
  ...
  bot.callbackQuery("mcp:restart", async (ctx) => {
    ...
    await ctx.editMessageText("\u{1F504} Restarting the OpenCode agent to apply MCP changes\u2026").catch(() => {});
    try {
